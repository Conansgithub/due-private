...
- 搜索已有工作: `openspec spec list --long`, `openspec list` (仅用 `rg` 进行全文搜索)
- 决定范围: 新功能 vs 修改已有功能
- 选择唯一的 `change-id`: kebab-case 命名,动词开头 (`add-`, `update-`, `remove-`, `refactor-`)
...
- 与以下之一配合: `create`, `plan`, `make`, `start`, `help`
...
2. 选择唯一的动词开头的 `change-id`,在 `openspec/changes/<id>/` 下创建 `proposal.md`, `tasks.md`, 可选的 `design.md`,以及规范增量。
3. 使用 `## ADDED|MODIFIED|REMOVED Requirements` 起草规范增量,每个需求至少包含一个 `#### Scenario:`。
4. 在共享提案之前运行 `openspec validate <id> --strict` 并解决所有问题。
...
4. **按顺序实施任务** - 按顺序完成
5. **确认完成** - 确保 `tasks.md` 中的每个项目都在更新状态之前完成
...
- 如果功能发生变化,更新 `specs/`
- 对于仅工具变更,使用 `openspec archive <change-id> --skip-specs --yes` (始终明确传递 change ID)
...
**迁移**: [如何处理]
```
如果影响多个功能,在 `changes/[change-id]/specs/<capability>/spec.md` 下为每个功能创建多个增量文件 - 每个功能一个。
...
- ADDED: 引入可独立作为需求的新功能或子功能。当变更是正交的 (例如,添加 "斜杠命令配置") 而不是改变现有需求的语义时,优先使用 ADDED。
- MODIFIED: 更改现有需求的行为、范围或验收标准。始终粘贴完整的、更新的需求内容 (标题 + 所有场景)。归档器将用您在此处提供的内容替换整个需求;部分增量将丢失以前的详细信息。
- RENAMED: 仅当名称更改时使用。如果您还更改行为,使用 RENAMED (名称) 加上引用新名称的 MODIFIED (内容)。

常见陷阱: 使用 MODIFIED 添加新关注点而不包括以前的文本。这会导致归档时丢失详细信息。如果您没有明确更改现有需求,而是在 ADDED 下添加新需求。
